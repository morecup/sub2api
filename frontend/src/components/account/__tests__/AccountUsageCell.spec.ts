...
describe('AccountUsageCell', () => {
  ...
  it.each([
    { name: 'clears a recovered forbidden state', snapshot: { is_forbidden: false, grok_entitlement_status: 'ACTIVE', grok_last_status_code: 200 } },
    { name: 'retains a server-reported forbidden state', snapshot: { is_forbidden: true, grok_entitlement_status: 'forbidden', grok_last_status_code: 403 } },
    { name: 'refreshes period-aligned paid usage', snapshot: { grok_billing: { period_type: 'weekly', usage_percent: 42 }, grok_local_usage_7d: { requests: 12, tokens: 1250000, cost: 3.25 }, grok_local_usage_24h: { requests: 3, tokens: 750000, cost: 0.75 } } }
  ])('Grok manual probe reloads the authoritative snapshot and $name', async ({ snapshot }) => {
    getUsage.mockResolvedValueOnce({ is_forbidden: true, grok_entitlement_status: 'forbidden' })
    const wrapper = mount(AccountUsageCell, {
      props: { account: makeAccount({ id: 4599, platform: 'grok', type: 'oauth', extra: {} }) },
      global: { stubs: { UsageProgressBar: true, AccountQuotaInfo: true, GrokQuotaProbeCell: true } }
    })
    await flushPromises()
    getUsage.mockResolvedValueOnce(snapshot)
    const state = wrapper.vm.$.setupState as {
      handleGrokProbed: () => Promise<void>
      usageInfo: Record<string, unknown> | null
    }
    await state.handleGrokProbed()
    await flushPromises()
    expect(getUsage).toHaveBeenLastCalledWith(4599, 'active', true)
    expect(state.usageInfo).toEqual(snapshot)
    wrapper.unmount()
  })
