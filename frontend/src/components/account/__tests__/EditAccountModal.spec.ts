...
describe('EditAccountModal', () => {
  ...
  it('submits OpenAI OAuth tool-frame-on-5h-exhausted toggle in extra', async () => {
    const account = buildAccount()
    account.type = 'oauth'
    account.credentials = {
  ...
  it('submits OpenAI OAuth force-tool-frame-after-5h toggle in extra', async () => {
    const account = buildAccount()
    account.type = 'oauth'
    account.credentials = {
      access_token: 'access-token',
    ...
    await wrapper.get('[data-testid="codex-tool-frame-5h-toggle"]').trigger('click')
    await wrapper.get('[data-testid="codex-tool-frame-force-after-5h-toggle"]').trigger('click')
    await wrapper.get('form#edit-account-form').trigger('submit.prevent')
  ...
  it('submits independent OpenAI OAuth 429 no-cooldown toggle in extra', async () => {
    const account = buildAccount()
    account.type = 'oauth'
    account.credentials = {
      access_token: 'access-token',
    ...
    await wrapper.get('[data-testid="openai-429-no-cooldown-toggle"]').trigger('click')
    await wrapper.get('form#edit-account-form').trigger('submit.prevent')

    expect(updateAccountMock).toHaveBeenCalledTimes(1)
    expect(updateAccountMock.mock.calls[0]?.[1]?.extra?.openai_429_no_cooldown).toBe(true)
    expect(updateAccountMock.mock.calls[0]?.[1]?.extra).not.toHaveProperty('codex_tool_frame_on_5h_exhausted')
  })
