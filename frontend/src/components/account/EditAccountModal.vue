    ...
    >
      ...
      >
        <div class="flex items-center justify-between gap-4">
          ...
          <button
            type="button"
            data-testid="openai-fixed-session-id-toggle"
            role="switch"
            :aria-checked="openAIFixedSessionIDEnabled"
            @click="openAIFixedSessionIDEnabled = !openAIFixedSessionIDEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.codexToolFrameOn5hExhausted') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          ...
          <button
            type="button"
            data-testid="codex-tool-frame-5h-toggle"
...
const openAIFixedSessionIDEnabled = ref(false)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  form.weight = newAccount.weight || 1
  ...
  openAIFixedSessionIDEnabled.value = false
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    openAIFixedSessionIDEnabled.value = newAccount.type === 'oauth' && extra?.openai_fixed_session_id_enabled === true
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      if (props.account.type === 'oauth' && openAIFixedSessionIDEnabled.value) {
        newExtra.openai_fixed_session_id_enabled = true
      } else {
        if (props.account.type === 'oauth') {
          newExtra.openai_fixed_session_id_enabled = false
        } else {
          delete newExtra.openai_fixed_session_id_enabled
        }
