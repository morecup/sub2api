...
const grokLocalUsage7d = computed(() => (
  usageInfo.value?.grok_local_usage_7d || usageInfo.value?.seven_day?.window_stats || usageInfo.value?.grok_local_usage || props.todayStats || null
))
...
// Probe results are persisted by the server. Reload the authoritative snapshot.
