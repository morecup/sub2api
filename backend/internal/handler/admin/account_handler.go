...
func NewAccountHandler(
	...
	rpmCache service.RPMCache,
	tokenCacheInvalidator service.TokenCacheInvalidator,
	codexDesktopServices ...*service.CodexDesktopAPIService,
) *AccountHandler {
	var codexDesktopService *service.CodexDesktopAPIService
	if len(codexDesktopServices) > 0 {
		codexDesktopService = codexDesktopServices[0]
	}
...
type CreateAccountRequest struct {
	...
	StandbyForAccountID     *int64         `json:"standby_for_account_id"`
	StandbyTriggerTypes     []string       `json:"standby_trigger_types"`
...
func (h *AccountHandler) Create(c *gin.Context) {
	...
	result, err := executeAdminIdempotent(c, "admin.accounts.create", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		account, execErr := h.adminService.CreateAccount(ctx, &service.CreateAccountInput{
			...
			StandbyForAccountID:   req.StandbyForAccountID,
			StandbyTriggerTypes:   req.StandbyTriggerTypes,
	...
	executeAdminIdempotentJSON(c, "admin.accounts.batch_create", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		...
		for _, item := range req.Accounts {
			...
			account, err := h.adminService.CreateAccount(ctx, &service.CreateAccountInput{
				...
				StandbyForAccountID:   item.StandbyForAccountID,
				StandbyTriggerTypes:   item.StandbyTriggerTypes,
				SkipMixedChannelCheck: skipCheck,
			})
			if err != nil {
				failed++
