...
func (h *OpenAIGatewayHandler) handleFailoverExhausted(c *gin.Context, failoverErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	if message := strings.TrimSpace(failoverErr.ClientMessage); message != "" {
		...
		} else if status == http.StatusBadRequest || status == http.StatusRequestEntityTooLarge {
...
func (h *OpenAIGatewayHandler) ensureForwardErrorResponse(c *gin.Context, streamStarted bool, errs ...error) bool {
	...
	if len(errs) > 0 {
		message = forwardErrorFallbackMessage(errs[0])
	}
