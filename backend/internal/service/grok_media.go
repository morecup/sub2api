...
func (s *OpenAIGatewayService) forwardGrokMediaVideoContent(
	...
	upstreamStart := time.Now()
	statusResp, err := s.doUpstreamRequest(statusReq, proxyURL, account)
	if err != nil {
		SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, false)
	}
	...
	contentResp, err := s.doUpstreamRequest(contentReq, proxyURL, account)
	SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
	if err != nil {
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, false)
	}
...
func (s *OpenAIGatewayService) handleGrokMediaErrorResponse(
	...
	_, fastTransient := classifyGrokFastTransientFailure(resp.StatusCode, body)
	...
	if !fastTransient && !account.ShouldHandleErrorCode(resp.StatusCode) {
	...
	kind := "http_error"
	if fastTransient || s.shouldFailoverGrokUpstreamError(resp.StatusCode, body) {
		kind = "failover"
	}
	appendGrokOpsUpstreamError(c, OpsUpstreamErrorEvent{
		Platform:           account.Platform,
	...
	if kind == "failover" {
		return nil, newGrokUpstreamFailoverError(account, resp.StatusCode, resp.Header, body, account.IsPoolMode() && account.IsPoolModeRetryableStatus(resp.StatusCode))
	}
