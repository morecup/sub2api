...
func TestFetchCodexModelsManifestPassthrough(t *testing.T) {
	...
	manifest, err := s.FetchCodexModelsManifest(context.Background(), newCodexModelsTestAccount(), "ignored-client-version", "")
	if err != nil {
	...
	if gotUserAgent != codexDesktopUserAgent {
		t.Errorf("user-agent header: got %q", gotUserAgent)
	}
}

func TestFetchCodexModelsManifestUsesFixedDesktopVersion(t *testing.T) {
...
func TestFetchCodexModelsManifestNotModified(t *testing.T) {
	...
	s := &OpenAIGatewayService{}
	manifest, err := s.FetchCodexModelsManifest(context.Background(), newCodexModelsTestAccount(), "ignored-client-version", `W/"abc123"`)
	if err != nil {
		t.Fatalf("FetchCodexModelsManifest returned error: %v", err)
	}
...
func TestFetchCodexModelsManifestMissingToken(t *testing.T) {
	...
	s := &OpenAIGatewayService{}
	if _, err := s.FetchCodexModelsManifest(context.Background(), account, "ignored-client-version", ""); err == nil {
