...
func TestOpenAIWSHTTPBridgeLaterTurn429RetriesCurrentTurnOnReplacementAccount(t *testing.T) {
	...
	require.Equal(t, resolveCodexSessionUUID(account.ID, 0, "client-session", ""), gjson.GetBytes(upstream.bodies[1], "client_metadata.session_id").String())
	require.Equal(t, resolveCodexSessionUUID(account.ID, 0, "client-session", ""), gjson.GetBytes(upstream.bodies[1], "client_metadata.thread_id").String())
	require.NotContains(t, string(upstream.bodies[2]), "previous_response_id")
	require.Contains(t, string(upstream.bodies[2]), "second")
	require.Equal(t, resolveCodexSessionUUID(nextAccount.ID, 0, "client-session", ""), gjson.GetBytes(upstream.bodies[2], "client_metadata.session_id").String())
	require.Equal(t, resolveCodexSessionUUID(nextAccount.ID, 0, "client-session", ""), gjson.GetBytes(upstream.bodies[2], "client_metadata.thread_id").String())
