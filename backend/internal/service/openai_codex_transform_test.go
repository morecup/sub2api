...
func TestApplyCodexOAuthTransform_NormalizesNativeToolCallPairsByType(t *testing.T) {
	...
	applyCodexOAuthTransformWithOptions(reqBody, codexOAuthTransformOptions{IsCodexCLI: false, SkipResponsesLiteSink: true})
...
func TestApplyCodexOAuthTransform_PreservesNativeCallIDsWhenRequested(t *testing.T) {
	...
	applyCodexOAuthTransformWithOptions(reqBody, codexOAuthTransformOptions{PreserveToolCallIDs: true, SkipResponsesLiteSink: true})
	...
	for _, placement := range []string{"top_level", "additional_tools"} {
		for _, mode := range []string{"auto", "required"} {
			t.Run(placement+"/"+mode, func(t *testing.T) {
				...
				result := applyCodexOAuthTransformWithOptions(reqBody, codexOAuthTransformOptions{IsCodexCLI: true, SkipResponsesLiteSink: true})
...
func TestApplyCodexOAuthTransform_StripsChatGPTInternalUnsupportedFields(t *testing.T) {
	...
	for _, field := range openAIChatGPTInternalUnsupportedFields {
		if field == "stream_options" {
			require.Equal(t, map[string]any{"reasoning_summary_delivery": codexStreamOptionsReasoningSummaryDelivery}, reqBody[field])
			continue
		}
...
func TestApplyCodexOAuthTransform_StripsMaxOutputTokensForCodexInternalAPI(t *testing.T) {
	reqBody := map[string]any{
		"model":             "gpt-5.5",
		"max_output_tokens": 64000,
		"input": []any{
			map[string]any{"role": "user", "content": "write a long report"},
		},
	}
