...
func normalizeOpenAIParallelToolCallsWithoutTools(body []byte, responsesLite bool) ([]byte, bool, error) {
	if responsesLite {
		...
		if parallel.Exists() && parallel.Type != gjson.True && parallel.Type != gjson.False {
			return body, false, nil
		}
...
func normalizeOpenAIResponsesWebSocketCompatibilityBody(body []byte, account *Account, responsesLite bool) ([]byte, bool, error) {
	...
	if account.IsOpenAIApiKey() || (usesCapturedCodexClientProfile(account) && isCodexResponsesLiteModel(gjson.GetBytes(normalized, "model").String()) && !openAIRequestBodyHasTools(normalized)) {
		if next, normalizedParallel, err := normalizeOpenAIParallelToolCallsWithoutTools(normalized, responsesLite || usesCapturedCodexClientProfile(account)); err != nil {
			return body, false, err
	...
	if account != nil && account.IsOpenAIOAuthLike() {
		...
		for _, field := range openAIChatGPTInternalUnsupportedFields {
			if field == "stream_options" && usesCapturedCodexClientProfile(account) && gjson.GetBytes(normalized, "stream_options.reasoning_summary_delivery").String() == codexStreamOptionsReasoningSummaryDelivery {
				continue
			}
...
func normalizeOpenAIPassthroughOAuthBody(body []byte, compact bool, installationID string) ([]byte, bool, error) {
	...
	if isCodexResponsesLiteModel(gjson.GetBytes(normalized, "model").String()) && !openAIRequestBodyHasTools(normalized) {
		next, pinned, pinErr := normalizeOpenAIParallelToolCallsWithoutTools(normalized, true)
		if pinErr != nil {
			return body, false, pinErr
		}
		normalized, changed = next, changed || pinned
	}
	return normalized, changed, nil
}

func detectOpenAIPassthroughInstructionsRejectReason(reqModel string, body []byte, responsesLite ...bool) string {
	if len(responsesLite) > 0 && responsesLite[0] {
		return ""
	}
