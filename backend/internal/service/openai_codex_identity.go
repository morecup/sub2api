...
// codexOutboundVersionPattern 允许 0.146.0 与 0.147.0-alpha.4 两类官方形态。
var codexOutboundVersionPattern = regexp.MustCompile(`^[0-9]+(\.[0-9]+){1,3}(-[0-9A-Za-z.]+)?$`)
...
func NormalizeCodexClientVersion(version string) string {
	...
	if version == "" || len(version) > codexClientVersionMaxLen || !codexOutboundVersionPattern.MatchString(version) {
		return ""
	}
	return version
}
