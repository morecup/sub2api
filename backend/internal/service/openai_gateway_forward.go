...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	skipFlattenForLiteSink := account.IsOpenAIOAuth() && !compactPath && !passthroughEnabled && !isOpenAICompatMessagesBridgeBody(body) && isCodexResponsesLiteModel(resolveOpenAIForwardUpstreamModel(account, strings.TrimSpace(gjson.GetBytes(body, "model").String()), isOpenAIResponsesCompactPath(c)))
	if (shouldFlattenOpenAIResponsesNamespaces(account, wsDecision.Transport, passthroughEnabled, compactPath) || isOpenAICompatMessagesBridgeBody(body)) && !skipFlattenForLiteSink && !(account.IsOpenAIOAuth() && passthroughEnabled && isOpenAIResponsesLiteHeader(c.GetHeader(responsesLiteHeader))) {
	...
	// Lite 契约下 input 项的 namespace 属有效语义（透传按入站 lite 头触发、合成
	// 按 lite 出站模型由 transform sink 处理），不是残留字段，不做剥离。
	liteInputNamespaceContract := skipFlattenForLiteSink ||
		(account.IsOpenAIOAuth() && passthroughEnabled && isOpenAIResponsesLiteHeader(c.GetHeader(responsesLiteHeader)))
	if shouldStripOpenAIResponsesInputNamespaces(account, wsDecision.Transport, passthroughEnabled) && !liteInputNamespaceContract {
	...
	if account.IsOpenAIApiKey() {
		if normalized, changed, normalizeErr := normalizeOpenAIParallelToolCallsWithoutTools(body, isCodexResponsesLiteModel(reqModel)); normalizeErr != nil {
	...
	if account.UsesOpenAICodexProtocol() {
		...
		if compatMessagesBridge {
			codexResult = applyCodexOAuthTransformWithOptions(decoded, codexOAuthTransformOptions{
				...
				SkipCapturedDesktopDefaults:         !usesCapturedCodexClientProfile(account),
				SkipResponsesLiteSink:               true,
			...
			codexResult = applyCodexOAuthTransformWithOptions(decoded, codexOAuthTransformOptions{
				IsCodexCLI:                          isCodexCLI,
				SkipResponsesLiteSink:               compactPath || !usesCapturedCodexClientProfile(account),
				IsCompact:                           isCompactRequest,
				SkipCapturedDesktopDefaults:         !usesCapturedCodexClientProfile(account),
				OmitPromotedSystemMessagesFromInput: omitPromotedSystemMessages,
			})
...
func (s *OpenAIGatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token string, isStream bool, promptCacheKey string, isCodexCLI bool) (*http.Request, error) {
	...
	if usesCapturedCodexClientProfile(account) {
		...
		if compatMessagesBridge {
			...
			fedRAMP := req.Header.Get("x-openai-fedramp")
			...
			if fedRAMP != "" {
				req.Header.Set("x-openai-fedramp", fedRAMP)
			}
	...
	if !codexMimicApplied {
		...
		// 若开启 ForceCodexCLI，则强制将上游 User-Agent 伪装为 Codex Desktop。
