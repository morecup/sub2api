...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	if account.Platform == PlatformGrok {
		upstreamReq, err = s.buildGrokResponsesRequest(upstreamCtx, c, account, responsesBody, token, grokCacheIdentity, s.cfg, s.settingService)
	} else {
	...
	if account.Platform != PlatformGrok && promptCacheKey != "" {
		...
		if fixed := account.GetOpenAIFixedSessionID(); fixed != "" {
			...
			if usesCapturedCodexClientProfile(account) || !account.UsesOpenAICodexProtocol() {
	...
	for attempt := 0; ; attempt++ {
		if attempt > 0 {
			...
			upstreamReq, err = s.buildGrokResponsesRequest(upstreamCtxRetry, c, account, responsesBody, token, grokCacheIdentity, s.cfg, s.settingService)
			releaseRetry()
			if err != nil {
	...
	if resp.StatusCode >= 400 {
		...
		return s.handleAnthropicErrorResponse(resp, c, account, responsesBody, billingModel)
	}
...
func (s *OpenAIGatewayService) handleAnthropicErrorResponse(
	...
	return s.handleCompatErrorResponse(resp, c, account, writeAnthropicError, requestBody, requestedModel...)
}
