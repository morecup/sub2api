...
// For OAuth accounts, oauth_401_set_temp_unschedulable_failed path may trigger
// SetTempUnschedulable; we just verify the bypass branch was not taken (i.e.,
// the function reached the OAuth 401 handler), which means status code 401 is
// processed normally.
func TestHandleUpstreamError_OAuthCodex401NotBypassed(t *testing.T) {
	...
	// OAuth 401 with detail=Unauthorized → handleAuthError → setErr + disable=true.
...
func TestHandleUpstreamError_OAuthCodex401Generic_GoesToTempUnschedulable(t *testing.T) {
	...
	account := &Account{
		...
		Type:        AccountTypeOAuth,
		Platform:    PlatformOpenAI,
		Credentials: map[string]any{"access_token": "tok", "expires_at": "2099-01-01T00:00:00Z"},
	}
...
func TestHandleUpstreamError_NonCodexUntouchedByBypass(t *testing.T) {
	t.Run("anthropic OAuth 429 still rate-limited", func(t *testing.T) {
		...
		// 429 without rate-limit headers + no anthropic match → falls through
		// to default 5m rate-limit branch.
		_ = svc.HandleUpstreamError(context.Background(), account, 429,
			http.Header{}, []byte(""))
		// Anthropic 429 without reset headers is skipped (returns early).
		// We mainly want to assert the OpenAI bypass branch was not taken,
		// which is implicit if the function didn't return false too early.
		// The key invariant: we DID reach the per-platform handle429 path.
		// (We don't assert exact call counts here because Anthropic without
		//  reset headers intentionally skips marking — that's existing behavior.)
		_ = repo
	})
	...
	t.Run("OpenAI APIKey 429 still rate-limited", func(t *testing.T) {
		...
		_ = svc.HandleUpstreamError(context.Background(), account, 429,
			http.Header{}, []byte(""))
		// OpenAI APIKey without reset info: handle429 → default 5m → SetRateLimited.
