...
func TestCodexRequestBodyIdentityNamespaceIsStablePerSetupAccount(t *testing.T) {
	...
	account11 := &Account{ID: 11, Platform: PlatformOpenAI, Type: AccountTypeSetupToken, Credentials: map[string]any{"chatgpt_account_id": "chatgpt-account-11"}}
	account19 := &Account{ID: 19, Platform: PlatformOpenAI, Type: AccountTypeSetupToken, Credentials: map[string]any{"chatgpt_account_id": "chatgpt-account-19"}}
...
func TestCodexAccountIdentitySourceResolvesShadowAndOverwritesFailoverContext(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, resolveCodexSessionUUID(shadow.ID, 0, "client-session", ""), req.Header.Get("session-id"))
...
func TestBuildOpenAIWSHeadersNamespacesCodexIdentityBySetupAccount(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
...
func TestBuildUpstreamRequestNamespacesCodexIdentityBySetupAccount(t *testing.T) {
	gin.SetMode(gin.TestMode)
