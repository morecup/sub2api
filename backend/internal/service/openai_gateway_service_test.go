...
func TestOpenAIHTTP429WithoutCodexToolFrameForceModeDoesNotCooldown(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-5.5","stream":true,"input":[{"type":"message","role":"user","content":"hello"},{"type":"function_call","call_id":"call_real","name":"read_file","arguments":"{}"},{"type":"function_call_output","call_id":"call_real","output":"done"}]}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")

	headers429 := http.Header{
		"Content-Type":                          []string{"application/json"},
		"X-Request-Id":                          []string{"rid_no_tool_frame_429"},
		"X-Codex-Primary-Used-Percent":          []string{"100"},
		"X-Codex-Primary-Window-Minutes":        []string{"10080"},
		"X-Codex-Secondary-Used-Percent":        []string{"100"},
		"X-Codex-Secondary-Window-Minutes":      []string{"300"},
	...
	upstream := &httpUpstreamRecorder{responses: []*http.Response{
		{
			StatusCode: http.StatusTooManyRequests,
			Header:     headers429,
			Body:       io.NopCloser(strings.NewReader(`{"error":{"type":"usage_limit_reached","message":"429 without tool frame"}}`)),
		},
	}}
	repo := &openAIToolFrame429AccountRepo{}
	svc := &OpenAIGatewayService{
...
func TestOpenAIStreamingErrorEventBeforeOutputServerOverloadedCodeReturnsFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
...
func TestOpenAIStreamingPassthroughErrorEventBeforeOutputServerOverloadedCodeReturnsFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
...
func TestOpenAIBuildUpstreamRequestOAuthOfficialClientOriginatorCompatibility(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, codexClientProfileForAccount(account).UserAgent(), req.Header.Get("User-Agent"))
			require.Equal(t, codexClientProfileForAccount(account).CodexVersion, req.Header.Get("version"))
		})
	}
}
...
func TestHandleCompatErrorResponseCyberPolicyEarlyReturn(t *testing.T) {
	...
	_, err := svc.handleCompatErrorResponse(resp, c, &Account{ID: 1, Platform: PlatformOpenAI, Name: "a"}, writeError, nil)
	require.Error(t, err)
