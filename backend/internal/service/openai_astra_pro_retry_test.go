...
// the existing OAuth rejected-field retry fixture (input[2].status is rejected,
...
func TestOpenAIGatewayService_OAuthAstraProModeKeptAcrossRejectedFieldRetry(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{responses: []*http.Response{
		newOpenAIRejectedFieldTestResponse(http.StatusBadRequest, `{"error":{"code":"unknown_parameter","message":"Unknown parameter: 'input[2].status'.","param":"input[2].status"}}`),
	...
	require.Equal(t, "completed", gjson.GetBytes(upstream.bodies[0], "input.2.status").String())
