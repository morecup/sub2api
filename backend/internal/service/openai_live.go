...
func (s *OpenAIGatewayService) createUpstreamLiveCall(
	...
	applyLiveUpstreamIdentityHeaders(upstreamReq.Header, account.GetOpenAIFixedSessionID(), account)
...
func applyLiveUpstreamIdentityHeaders(headers http.Header, fixedSessionID string, accounts ...*Account) {
	var account *Account
	if len(accounts) > 0 {
		account = accounts[0]
	}
...
func (s *OpenAIGatewayService) liveSidebandHeaders(
	...
	applyLiveUpstreamIdentityHeaders(headers, account.GetOpenAIFixedSessionID(), account)
	return headers, nil
}
