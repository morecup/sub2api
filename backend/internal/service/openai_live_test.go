...
func TestCreateUpstreamLiveCallPreservesSession(t *testing.T) {
	...
	require.Equal(t, "codex_cli_rs/0.144.1 (Ubuntu 22.4.0; x86_64) xterm-256color", upstream.request.Header.Get("User-Agent"))
	require.Equal(t, "codex_cli_rs", upstream.request.Header.Get("originator"))
	require.Equal(t, "0.144.1", upstream.request.Header.Get("version"))
