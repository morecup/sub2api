...
func TestAccountTestService_TestAccountConnection_OpenAICompactProbeIdentityMatchesRealTraffic(t *testing.T) {
	...
	// Even imported convergence settings must preserve the captured OAuth profile.
	profile := codexClientProfileForAccount(&account)
	session := resolveCodexSessionUUID(account.ID, 0, compactProbeSessionID(account.ID), "")
	require.Equal(t, session, upstream.lastReq.Header.Get("session-id"))
	require.Equal(t, session, upstream.lastReq.Header.Get("thread-id"))
	require.Empty(t, upstream.lastReq.Header.Get("session_id"))
	require.Equal(t, profile.InstallationID, gjson.Get(upstream.lastReq.Header.Get("x-codex-turn-metadata"), "installation_id").String())
