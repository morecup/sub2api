...
func TestForwardAsChatCompletions_UpstreamRequestIgnoresClientCancel(t *testing.T) {
	...
	require.NotNil(t, result)
	require.NotNil(t, upstream.lastReq)
	require.NoError(t, upstream.requestContextErr, "client cancellation must not cancel the upstream request")
	require.ErrorIs(t, upstream.lastReq.Context().Err(), context.Canceled, "completed attempts release their context")
}
