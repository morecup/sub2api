...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	routingModel, routingServiceTier := "", ""
	if len(routingArgs) > 0 {
		routingModel = routingArgs[0]
	}
	if len(routingArgs) > 1 {
		routingServiceTier = routingArgs[1]
	}
	...
	if !isOAuthAccount && account != nil && account.UsesOpenAICodexProtocol() {
	...
	if isOAuthAccount {
		...
		if fedRAMP != "" {
			headers.Set("x-openai-fedramp", fedRAMP)
		}
