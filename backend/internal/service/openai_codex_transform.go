...
type codexOAuthTransformOptions struct {
	...
	SkipCapturedDesktopDefaults bool
}
...
func applyCodexOAuthTransformWithOptions(reqBody map[string]any, opts codexOAuthTransformOptions) codexTransformResult {
	...
	if opts.IsCompact {
		...
		if v, ok := reqBody["stream_options"].(map[string]any); !opts.SkipCapturedDesktopDefaults && (!ok || len(v) != 1 || v["reasoning_summary_delivery"] != codexStreamOptionsReasoningSummaryDelivery) {
	...
	for _, key := range openAICodexOAuthUnsupportedFields {
		// The captured Desktop protocol requires the normalized delivery option
		// above. Legacy compact requests still strip stream_options entirely.
		if key == "stream_options" && !opts.IsCompact && !opts.SkipCapturedDesktopDefaults {
			continue
		}
