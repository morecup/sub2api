...
func (s *OpenAIGatewayService) isOpenAICompatibleAccountEligibleForRequest(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) bool {
	...
	return accountSchedulableWithStandby(ctx, account, s.lookupStandbyPrimaryAccount)
}

func (s *OpenAIGatewayService) lookupStandbyPrimaryAccount(ctx context.Context, accountID int64) (*Account, error) {
...
func shouldAutoPauseGrokAccountByQuotaAt(account *Account, now time.Time) (bool, openAIQuotaAutoPauseDecision) {
...
func shouldAutoPauseOpenAIAccountByQuota(ctx context.Context, account *Account) (bool, openAIQuotaAutoPauseDecision) {
	...
	now := time.Now()
	ignoreQuotaAutoPauseForToolFrame := account.IsOpenAIOAuth() && resolveAccountExtraBool(account.Extra, openAICodexToolFrameOn5hExhaustedKey)
	if !ignoreQuotaAutoPauseForToolFrame && !disabled5h && threshold5h > 0 {
	...
	if !ignoreQuotaAutoPauseForToolFrame && !disabled7d && threshold7d > 0 {
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool) (*AccountSelectionResult, error) {
	...
	for i := range accounts {
		...
		if acc.HasStandbyConfiguration() && !accountSchedulableWithStandby(ctx, acc, s.lookupStandbyPrimaryAccount) {
			filterStats.exclude("standby")
			continue
		}
...
func (s *OpenAIGatewayService) resolveFreshSchedulableOpenAIAccountBeforeProfit(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if !accountSchedulableWithStandby(ctx, fresh, s.lookupStandbyPrimaryAccount) || !isOpenAICompatibleAccountEligibleForRequestBeforeProfit(ctx, fresh, platform, requestedModel, requireCompact, requiredCapability) {
		return nil
	}
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBBeforeProfit(ctx context.Context, account *Account, groupID *int64, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if s.schedulerSnapshot == nil || s.accountRepo == nil {
		...
		if !accountSchedulableWithStandby(ctx, account, s.lookupStandbyPrimaryAccount) || !isOpenAICompatibleAccountEligibleForRequestBeforeProfit(ctx, account, platform, requestedModel, requireCompact, requiredCapability) {
			return nil
		}
	...
	if !accountSchedulableWithStandby(ctx, latest, s.lookupStandbyPrimaryAccount) || !isOpenAICompatibleAccountEligibleForRequestBeforeProfit(ctx, latest, platform, requestedModel, requireCompact, requiredCapability) {
		return nil
	}
