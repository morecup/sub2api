...
func newOpenAIImagesTestService(upstream HTTPUpstream) *OpenAIGatewayService {
	return &OpenAIGatewayService{
		...
		cfg: &config.Config{
			// Image fixtures assert model traffic only; telemetry has its own transport tests.
			Gateway: config.GatewayConfig{CodexTelemetry: config.CodexTelemetryConfig{Mode: "off"}},
