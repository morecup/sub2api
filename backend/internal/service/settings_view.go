...
type SystemSettings struct {
	...
	EnableCCHSigning                       bool   // 兼容旧配置；CCH 会按 claude.exe native 层算法自动补写
	EnableClaudeOAuthSystemPromptInjection bool   // 兼容旧配置；Claude OAuth mimic 路径现在强制重建 Claude Code system blocks
	...
	OpenAICodexTicketEnabled               bool   // Codex 打票总开关；关闭则不打票不注入
	OpenAICodexTicketHarvestProxyURL       string // Codex 打票代理 URL；空则回退 yaml/env
