...
func stagedCodexFingerprintIDs(c *gin.Context, account *Account) *codexFingerprintIDs {
	if c == nil || account == nil || !account.UsesOpenAICodexProtocol() || usesCapturedCodexClientProfile(account) {
		return nil
	}
...
func (a *Account) GetCodexFingerprintMode() codexFingerprintMode {
	// Persisted Desktop profiles own OAuth identity, including installation,
	// session, thread, turn and window IDs. Ignore imported convergence settings
	// rather than layering a second identity policy over the captured protocol.
	if a == nil || !a.IsOpenAIOAuthLike() || usesCapturedCodexClientProfile(a) {
		return codexFingerprintOff
	}
...
func resolveConvergedInstallationID(account *Account, seed string) string {
	...
	if deviceID := account.GetExtraString("openai_device_id"); deviceID != "" {
		return deviceID
	}
	if seed == "" {
		return ""
...
func resolveCodexFingerprintIDs(account *Account, clientSessionID string, mode codexFingerprintMode) *codexFingerprintIDs {
	if account == nil || mode == codexFingerprintOff || usesCapturedCodexClientProfile(account) {
		return nil
	}
