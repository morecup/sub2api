...
func appendOpenAICompatClaudeCodeTodoGuard(req *apicompat.ResponsesRequest) bool {
	...
	if len(items) == 0 ||
		responsesInputItemsEndWithCompactionTrigger(items) ||
...
func appendOpenAICompatClaudeCodeTodoGuardToRequestBody(reqBody map[string]any) bool {
	...
	if !ok ||
		len(input) == 0 ||
		isCodexRemoteCompactionV2Input(input) ||
...
func responsesInputItemsEndWithCompactionTrigger(items []apicompat.ResponsesInputItem) bool {
	if len(items) == 0 {
		return false
	}
