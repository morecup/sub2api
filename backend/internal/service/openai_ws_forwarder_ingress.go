...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	wsHeaders, _, buildHdrErr := s.buildOpenAIWSHeaders(
		...
		isCodexCLI,
		turnState,
		openAIWSFrameTurnMetadata(firstPayload.payloadRaw, strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader))),
	...
	for {
		...
		if nextPayload.promptCacheKey != "" {
			...
			updatedHeaders, _, updHdrErr := s.buildOpenAIWSHeaders(
				...
				isCodexCLI,
				turnState,
				openAIWSFrameTurnMetadata(nextPayload.payloadRaw, strings.TrimSpace(c.GetHeader(openAIWSTurnMetadataHeader))),
