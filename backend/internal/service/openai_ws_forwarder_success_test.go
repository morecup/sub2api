...
func TestOpenAIGatewayService_BuildOpenAIWSHeadersDeviceModePreservesNamespacedClientSessionIdentity(t *testing.T) {
	...
	account := newTestConvergenceSetupAccount(1300, map[string]any{codexFingerprintModeExtraKey: "device"})
...
func TestOpenAIGatewayService_Forward_WSv2_OAuthStoreFalseByDefault(t *testing.T) {
	...
	body := []byte(`{"model":"gpt-5.1","stream":false,"store":true,"prompt_cache_key":"sess-oauth-1","input":[{"type":"input_text","text":"hello","namespace":"native-wsv2"}]}`)
	result, err := svc.Forward(context.Background(), c, account, body)
	require.NoError(t, err)
	require.NotNil(t, result)
	...
	require.Equal(t, "realtime_conversation,remote_compaction_v2", captureDialer.lastHeaders.Get("x-codex-beta-features"))
	// OAuth 账号的 session_id/conversation_id 应同时按 API key 和上游账号隔离，
	// 测试中未设置 api_key 到 context，apiKeyID=0。
	require.Equal(t, resolveCodexSessionUUID(account.ID, 0, "sess-oauth-1", ""), captureDialer.lastHeaders.Get("session-id"))
...
func TestOpenAIGatewayService_Forward_WSv2_OAuthSanitizesInvalidNativeToolItemID(t *testing.T) {
	...
	require.False(t, gjson.Get(requestJSON, `input.#(type=="custom_tool_call").id`).Exists(), "stale fc_* ID must not be replayed as a native custom_tool_call ID")
	require.Equal(t, "ctc_hotfix", gjson.Get(requestJSON, `input.#(type=="custom_tool_call").call_id`).String())
	require.Equal(t, "custom_tool_call_output", gjson.Get(requestJSON, `input.#(type=="custom_tool_call_output").type`).String())
	require.Equal(t, "ctc_hotfix", gjson.Get(requestJSON, `input.#(type=="custom_tool_call_output").call_id`).String())
}
...
func TestOpenAIGatewayService_Forward_WSv2_OAuthOriginatorCompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "desktop originator ignored", originator: "Codex Desktop"},
		{name: "vscode originator ignored", originator: "codex_vscode"},
		{name: "official ua ignored", userAgent: "Codex Desktop/1.2.3"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.NotNil(t, result)
			require.Equal(t, codexDesktopOriginator, captureDialer.lastHeaders.Get("originator"))
			require.Equal(t, codexClientProfileForAccount(account).UserAgent(), captureDialer.lastHeaders.Get("user-agent"))
			require.Equal(t, codexClientProfileForAccount(account).CodexVersion, captureDialer.lastHeaders.Get("version"))
		})
	}
}

func TestOpenAIGatewayService_BuildOpenAIWSHeaders_OAuthFinalMimicSanitizesInbound(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	c.Request.Header.Set("session_id", "sess-ws-sanitize")
	c.Request.Header.Set("conversation_id", "conv-should-not-pass")
	c.Request.Header.Set("accept-language", "zh-CN")
	c.Request.Header.Set("version", "999.0")
	c.Request.Header.Set(openAIWSTurnStateHeader, "spoof-turn-state")
	c.Request.Header.Set(openAIWSTurnMetadataHeader, `{"spoof":true}`)
	...
	account := &Account{
		...
		Credentials: map[string]any{
			...
			"chatgpt_account_id": "acct-ws-sanitize",
	...
	require.Equal(t, wantSessionID+":0", headers.Get("x-codex-window-id"))
...
func TestOpenAIGatewayService_Forward_WSv2_OAuthUsesCapturedProfile(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Equal(t, codexDesktopOriginator, captureDialer.lastHeaders.Get("originator"))
	require.Equal(t,
		codexClientProfileForAccount(account).UserAgent(),
...
func TestOpenAIGatewayService_Forward_WSv2_CodexFingerprintHandshakeBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	account := newTestConvergenceSetupAccount(4405, map[string]any{
