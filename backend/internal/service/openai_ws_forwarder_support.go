...
func (s *OpenAIGatewayService) resolveAccountByPreviousResponseIDForCapability(
	...
	if shouldClearStickySession(account, requestedModel) || !account.IsOpenAI() ||
		!accountSchedulableWithStandby(ctx, account, s.lookupStandbyPrimaryAccount) {
		_ = store.DeleteResponseAccount(ctx, derefGroupID(groupID), responseID)
		return 0, nil, "", nil
	}
	...
	if s.schedulerSnapshot != nil && s.accountRepo != nil {
		...
		if shouldClearStickySession(latest, requestedModel) || !latest.IsOpenAI() ||
			!accountSchedulableWithStandby(ctx, latest, s.lookupStandbyPrimaryAccount) {
			_ = store.DeleteResponseAccount(ctx, derefGroupID(groupID), responseID)
			return 0, nil, "", nil
		}
