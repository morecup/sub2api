...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	buildRequest := func(body []byte) (*http.Request, error) {
		...
		if isOAuth {
			...
			applyCodexOAuthMimicHeadersForAccount(req, account, 0, sessionSeed, account.GetOpenAIFixedSessionID(), codexDesktopOriginator, false, isCodexResponsesLiteModel(upstreamTestModelID), upstreamTestModelID)
		...
		if isOAuth {
			// 与真实转发一致：originator 与最终 User-Agent 首段配套。
			enforceCodexIdentityHeadersForAccount(req.Header, account)
		}
		return req, nil
	}
...
func (s *AccountTestService) testGrokRealtime(c *gin.Context, ctx context.Context, account *Account, authToken, modelID string) error {
	...
	conn, status, _, dialErr := dialer.Dial(dialCtx, wsURL, headers, s.grokTestProxyURL(account), nil, "")
	if dialErr != nil {
...
func (s *AccountTestService) testOpenAICompactConnection(c *gin.Context, account *Account, testModelID string) error {
	...
	if isOAuth {
		req.Host = "chatgpt.com"
		applyCodexOAuthMimicHeadersForAccount(req, account, 0, probeSessionID, account.GetOpenAIFixedSessionID(), codexDesktopOriginator, false, isCodexResponsesLiteModel(testModelID), testModelID)
		applyCodexDesktopOptionalCookie(req.Header, credentialAccount)
	...
	// Native remote compaction uses the regular Responses stream and its beta.
	ensureOpenAIRemoteCompactionV2BetaFeature(req.Header)
	// 账号级请求头覆写：测试请求与真实转发保持一致的最终头
	account.ApplyHeaderOverrides(req.Header)
...
func (s *AccountTestService) testOpenAIImageOAuth(c *gin.Context, ctx context.Context, account *Account, modelID, prompt string) error {
	...
	if nextBody, changed := appendCodexToolFrameIfNeeded(requestBody); !direct && changed {
		requestBody = nextBody
		s.sendEvent(c, TestEvent{Type: "status", Text: "OpenAI OAuth 测试请求已启用 Tool Frame"})
	}
	// lite 探测模型的 body 同步下沉，与 lite 头契约一致（非 lite 原样返回）。
	...
	buildRequest := func(body []byte) (*http.Request, error) {
		...
		applyCodexOAuthMimicHeadersForAccount(req, account, 0, parsed.StickySessionSeed(), account.GetOpenAIFixedSessionID(), codexDesktopOriginator, false, isCodexResponsesLiteModel(parsed.Model), parsed.Model)
		applyCodexDesktopOptionalCookie(req.Header, credentialAccount)
		if !direct {
			body, err = syncCodexOAuthMimicRequestBody(req, body, false)
			if err != nil {
