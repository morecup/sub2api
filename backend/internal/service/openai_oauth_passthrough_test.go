...
func TestOpenAIGatewayService_OAuthMessagesBridgeDoesNotInjectDefaultInstructions(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	originalBody := []byte(`{"model":"gpt-5.6-terra","stream":true,"prompt_cache_key":"anthropic-metadata-session-1","tools":[{"type":"namespace","name":"collaboration","tools":[{"type":"function","name":"spawn_agent"}]}],"input":[{"type":"message","role":"developer","content":[{"type":"input_text","text":"<sub2api-claude-code-todo-guard>"}]},{"type":"message","role":"user","content":"hello"}]}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(originalBody))
	c.Request.Header.Set("Content-Type", "application/json")
...
func TestOpenAIGatewayService_OAuthPassthrough_NonStreamingClientUsesStreamingUpstreamAndReturnsJSON(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"gpt-5.2","stream":false,"store":true,"instructions":"answer briefly","input":[{"type":"message","role":"user","content":"hi"}]}`)
	upstreamSSE := strings.Join([]string{
		`data: {"type":"response.output_text.delta","delta":"ok"}`,
		"",
		`data: {"type":"response.completed","response":{"id":"resp_forced_stream","object":"response","model":"gpt-5.2","status":"completed","output":[{"type":"message","id":"msg_1","role":"assistant","status":"completed","content":[{"type":"output_text","text":"ok"}]}],"usage":{"input_tokens":3,"output_tokens":1,"total_tokens":4}}}`,
		"",
		"data: [DONE]",
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_forced_stream"}},
		Body:       io.NopCloser(strings.NewReader(upstreamSSE)),
	}}
	...
	require.Equal(t, "resp_forced_stream", gjson.Get(rec.Body.String(), "id").String())
...
func TestOpenAIGatewayService_OAuthPassthrough_NamespaceRequestAndStreamResponse(t *testing.T) {
	...
	for _, liteMarker := range []bool{false, true} {
		name := "无 lite 标记保持摊平"
		if liteMarker {
			name = "带 lite 标记走 additional_tools"
		}
		t.Run(name, func(t *testing.T) {
			...
			upstreamToolNamespace := ""
			if liteMarker {
				upstreamToolName = "spawn_agent"
				upstreamToolNamespace = `,"namespace":"collaboration"`
			}
			upstreamSSE := strings.Join([]string{
				`data: {"type":"response.output_item.added","output_index":0,"item":{"type":"function_call","id":"fc_1","call_id":"call_1","name":"` + upstreamToolName + `"` + upstreamToolNamespace + `,"arguments":""}}`,
				"",
				`data: {"type":"response.output_item.done","output_index":0,"item":{"type":"function_call","id":"fc_1","call_id":"call_1","name":"` + upstreamToolName + `"` + upstreamToolNamespace + `,"arguments":"{}"}}`,
				"",
				`data: {"type":"response.completed","response":{"id":"resp_1","status":"completed","output":[{"type":"function_call","id":"fc_1","call_id":"call_1","name":"` + upstreamToolName + `"` + upstreamToolNamespace + `,"arguments":"{}"}],"usage":{"input_tokens":2,"output_tokens":1,"total_tokens":3}}}`,
				"",
				"data: [DONE]",
				"",
			}, "\n")
			...
			account := &Account{
				...
				Credentials: map[string]any{"access_token": "oauth-token", "chatgpt_account_id": "chatgpt-acc"},
				Extra:       map[string]any{"openai_passthrough": true, "openai_responses_flatten_namespaces": true}, Status: StatusActive, Schedulable: true, RateMultiplier: f64p(1),
			}
			...
			if liteMarker {
				// 入站带 lite 标记：透传按 Lite 合约归一化，namespace 工具走
...
func TestOpenAIGatewayService_NativeOAuth_FlattenEnabledNamespaceRequestAndStreamResponse(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	// 非 lite 出站模型（gpt-5.5）不做任何 lite 归一化：无 reasoning.context、无 lite 头；
	// namespace 工具维持既有摊平行为（collaboration__spawn_agent），下行再映射回 namespace 形态。
	require.False(t, gjson.GetBytes(upstream.lastBody, "reasoning.context").Exists())
	require.Empty(t, upstream.lastReq.Header.Get(responsesLiteHeader))
...
// namespace 摊平冲突按入站 lite 标记分支：无标记时维持本地 400（摊平后的
// collaboration__spawn_agent 与既有顶层 function 冲突）；带标记时按 Lite 合约
// 把 namespace 迁入 input.additional_tools，不再产生摊平冲突。
func TestOpenAIGatewayService_OAuthPassthrough_NamespaceConflictByLiteMarker(t *testing.T) {
	gin.SetMode(gin.TestMode)

	for _, liteMarker := range []bool{false, true} {
		name := "无 lite 标记摊平冲突 400"
		if liteMarker {
			name = "带 lite 标记迁入 additional_tools 放行"
		}
		t.Run(name, func(t *testing.T) {
			rec := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(rec)
...
func TestOpenAIGatewayService_Forward_MissingInstructionsUsesMappedModelTemplate(t *testing.T) {
	...
	instructions := gjson.GetBytes(upstream.lastBody, "input.1.content.0.text").String()
...
func TestOpenAIGatewayService_OAuthPassthrough_CodexTuiIdentityRebuiltAsDesktop(t *testing.T) {
	...
	require.Equal(t, codexClientProfileForAccount(account).CodexVersion, upstream.lastReq.Header.Get("version"))
}
...
func TestOpenAIGatewayService_CodexFingerprintHTTPTransformedHeaderBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	account := newTestConvergenceSetupAccount(4401, map[string]any{codexFingerprintModeExtraKey: "session"})
...
func TestOpenAIGatewayService_CodexFingerprintHTTPRawPassthroughHeaderBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	account := newTestConvergenceSetupAccount(4402, map[string]any{
...
func TestOpenAIGatewayService_CodexFingerprintCompactDoesNotRewriteBodyCacheKeyOrMetadata(t *testing.T) {
	...
	account := newTestConvergenceSetupAccount(4403, map[string]any{codexFingerprintModeExtraKey: "session"})
...
func TestOpenAIGatewayService_CodexFingerprintMessagesBridgeDoesNotInjectBodyPromptCacheKey(t *testing.T) {
	...
	account := newTestConvergenceSetupAccount(4404, map[string]any{codexFingerprintModeExtraKey: "session"})
