...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	if statusCode == http.StatusTooManyRequests && shouldSuppressOpenAI429AccountCooldown(account) {
		slog.Warn("rate_limit_429_account_cooldown_skipped",
			"account_id", account.ID,
	...
	case 401:
		// Spark 影子账号的鉴权状态必须落到实际持有凭据的母账号。
		authAccount := account
		if resolved, rerr := resolveCredentialAccount(ctx, s.accountRepo, account); rerr == nil && resolved != nil {
			authAccount = resolved
		}

		// 普通 OAuth 缺少 refresh_token 时默认不处理；Antigravity 无法自行恢复，标记鉴权错误。
		// 只有账号显式开启 credentials.oauth_401_no_refresh_token_set_error 时才恢复旧的 SetError 行为。
		if authAccount.Type == AccountTypeOAuth && strings.TrimSpace(authAccount.GetCredential("refresh_token")) == "" {
			if authAccount.Platform != PlatformAntigravity && !authAccount.ShouldSetErrorOnOAuth401NoRefreshToken() {
				slog.Info("oauth_401_no_refresh_token_ignored", "account_id", authAccount.ID, "platform", authAccount.Platform)
				break
			}
			if authAccount.Platform == PlatformAntigravity && s.tokenCacheInvalidator != nil {
